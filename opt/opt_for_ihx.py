...
try:
    from ihx import evaluate_ihx_cycle
except ImportError:
    print("="*50)
    print("Error: Could not find 'ihx.py'.")
    print("="*50)
    exit()

T_source_in = 75.0

class ihxProblem(Problem):
    ...
    def __init__(self, fixed_params):
        ...
        epsilon_min = 0.01   # no IHX
        epsilon_max = 0.99  # Prevent eps = 1 
        
        xl = np.array([p_evap_min_Pa, p_cond_min_Pa, epsilon_min])
        xu = np.array([p_evap_max_Pa, p_cond_max_Pa, epsilon_max])
        
        # Initialize the Pymoo Problem class
        super().__init__(n_var=3,       # 3 Variables (P_evap, P_cond, epsilon_ihx)
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        for individual_inputs in x:
            # individual_inputs are [p_evap, p_cond, epsilon_ihx]
            
            neg_COP, Total_UA, T_evap_K, T_cond_K = \
                evaluate_ihx_cycle(individual_inputs, self.fixed_params)
...
if __name__ == "__main__":
    ...
    problem = ihxProblem(fixed_params=fixed_params)
    ...
    # 5. Run Optimization
    print("="*50)
    print(f"Optimization Start (IHX Model)")
    ...
    # 6. Extract results
    variables = res.X  # optimal variables [P_evap, P_cond, Epsilon_IHX]
    objectives = res.F # optimal objectives [neg_COP, Total_UA]

    
    # 7. Print results
    print("\n--- Optimal Solutions (Pareto Front) ---")
    print(f"{'Sol #':>5} | {'T_evap (C)':>10} | {'T_cond (C)':>10} | {'Eps_IHX':>8} | {'COP':>8} | {'Total UA (W/K)':>15}")
    print("-" * 69)
    ...
    if variables is None or objectives is None:
        ...
        for i in range(len(variables)):
            p_evap, p_cond, epsilon_ihx = variables[i, 0],  variables[i, 1], variables[i, 2]
            ...
            try:
                ...
                results_list.append([p_evap, p_cond, t_evap_c, t_cond_c, epsilon_ihx, cop, ua])
            except ValueError:
                continue 
        ...
        if not results_list:
            ...
            df_results = pd.DataFrame(results_list, columns=['P_evap','P_cond','T_evap_C', 'T_cond_C', 'Eps_IHX', 'COP', 'Total_UA_W_K'])
    ...
    if valid_solutions_found:
        ...
        print(f"\n  [Best Solution based on weights] when T_source_in : {T_source_in:.2f} (C)")
